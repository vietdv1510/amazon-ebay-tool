      ...
      <div class="grid-area mx-4 mt-4 h-full" :class="{ 'with-panel': selectedRow }">
        ...
        >
          ...
          <p class="text-muted-foreground text-sm max-w-sm">
            Sử dụng Amazon Crawler để crawl dữ liệu trước khi xử lý tại đây.
          </p>
        </div>

        <div
          v-else
          class="flex-1 h-full overflow-hidden flex flex-col bg-card rounded-t-xl ring-1 ring-border/50 shadow-sm border-b-0"
        >
          <Table class="relative w-full" wrapperClass="flex-1 min-h-0">
            <TableHeader class="sticky top-0 bg-muted/80 backdrop-blur z-10">
              <TableRow class="hover:bg-transparent">
                <TableHead class="w-[40px] text-center font-semibold whitespace-nowrap"
                  >#</TableHead
                >
                ...
                <TableHead class="w-[85px] text-right font-semibold whitespace-nowrap"
                  >Giá gốc</TableHead
                >
                <TableHead class="w-[85px] text-right font-semibold whitespace-nowrap"
                  >Giá eBay</TableHead
                >
                <TableHead class="w-[70px] text-center font-semibold whitespace-nowrap"
                  >Biến thể</TableHead
                >
                <TableHead class="w-[150px] font-semibold whitespace-nowrap">Category</TableHead>
                <TableHead class="w-[100px] text-center font-semibold whitespace-nowrap"
              ...
              >
                ...
                <TableCell>
                  <div class="flex items-start gap-2 max-w-[260px] whitespace-normal">
                    <div
                      class="w-8 h-8 rounded border bg-muted/50 flex-shrink-0 overflow-hidden mt-0.5"
                    >
                      <img
                ...
                <TableCell
                  class="text-right text-sm font-semibold text-green-600 whitespace-nowrap"
                >
                ...
                <TableCell class="whitespace-nowrap">
                  <div v-if="row.ebayCategory" class="flex items-center gap-1.5">
                    ...
                    <span class="text-xs truncate max-w-[120px]" :title="row.ebayCategoryName">{{
                      row.ebayCategoryName || row.ebayCategory
                    }}</span>
                  </div>
...
const onRowClick = (row) => {
  ...
  if (detailPanelRef.value?.isDirty) {
    if (
      !confirm(
        'Sản phẩm hiện tại có thay đổi chưa lưu. Bạn có chắc chắn muốn chuyển sang sản phẩm khác?'
      )
    ) {
      return
    }
